    ...
    for test_index = index+index_offset:size(y_test,1)
        if y_test(test_index) == 1
            ...
            for bbox_index = 1:4:size(bounding_boxes,2)
                rectangle('Position',...
                    [bounding_boxes(gbox_index+1),...
                    bounding_boxes(gbox_index),...
                    bounding_boxes(gbox_index+3)-bounding_boxes(gbox_index+1),...
                    bounding_boxes(gbox_index+2)-bounding_boxes(gbox_index)],...
                    'EdgeColor','b');
                hold on;
            end
